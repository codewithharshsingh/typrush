document.addEventListener("DOMContentLoaded", () => {
  ...
  let timeElapsed = 0;
  let charIndex = 0;
  let totalCharsInTest = 0;
  let isMuted = false;

  const urlParams = new URLSearchParams(window.location.search);
  const testValue = parseInt(urlParams.get("value")) || 25;
  ...
  function loadNewText() {
    ...
    const shuffledText = textArray
      ...
      .slice(0, testValue)
      .join(" ");
    totalCharsInTest = shuffledText.length;
  ...
  function updateProgress() {
    const progressPercentage =
      totalCharsInTest > 0 ? (charIndex / totalCharsInTest) * 100 : 0;
